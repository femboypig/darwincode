...
- **Локальное шифрование**: Настройки (`config.json`) и истории сессий в каталоге `~/.config/darwincode/` симметрично зашифрованы с помощью AES-256-GCM с использованием безопасного ключа, полученного из системной связки ключей (или сгенерированного случайно и сохраненного локально с правами доступа к файлу `0600`).

### Безопасность пользовательских команд рабочей области

По умолчанию глобальные пользовательские команды, определенные в каталоге `~/.config/darwincode/commands/`, считаются доверенными автоматически. Локальные команды проекта, расположенные в `.darwincode/commands/*.toml`, считаются недоверенными.

Чтобы запустить недоверенные команды проекта, вы должны:
* Установить `"trust_workspace": true` в вашем глобальном файле настроек.
* Явно разрешить выполнение команд для текущего сеанса, запустив программу с CLI-флагом `--trust-workspace`:
  ```bash
  darwincode --trust-workspace
  ```
* Подтвердить выполнение во всплывающем интерактивном окне в TUI при вызове команды.
